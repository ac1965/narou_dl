...
修正履歴::
    ...
    テスト追加時に発覚: _serialize_paragraph() が p_tag.get_text() の
    有無だけで空行判定していたため、<p><img .../></p> のようなテキストを
    伴わない挿絵のみの段落が全て空行として扱われ、挿絵そのものが
    Episode.paragraphs から消えていた(実際にキャッシュ済みの全作品で
    挿絵が1件も保存されていないことから発覚)。テキストが無くても<img>を
    含む場合は空行としない判定に修正した。
"""
...
def _serialize_paragraph(p_tag: Tag) -> str:
    ...
    なろうの挿絵は本文中で <p><img .../></p> のように、テキストを一切
    伴わず単独の<p>として挿入されることが多い。<img>にはテキストノードが
    無いため p_tag.get_text() は空文字列になり、これだけで空行判定すると
    挿絵のみの段落が全て空行として扱われ、挿絵そのものが本文から消えて
    しまう不具合があった。テキストが無くても<img>を含む場合は空行とせず、
    通常通りシリアライズする。
    """
    if not p_tag.get_text(strip=True) and p_tag.find("img") is None:
        return ""
